...
1. Download the PLUTO+ firmware from the plutoplus repo: [Download plutosdr-fw.zip](https://github.com/plutoplus/plutoplus/releases)
2. Physically open up the PLUTO+ and move the DIP switches in the bottom right to be in the stock firmware position.
3. Plug in the PLUTO+ and drop the `pluto.frm` in the PLUTO device found in file explorer.
4. Press the eject button in the file explorer, and wait for the device to reappear.
5. SSH into the PLUTO+ (`ssh root@192.168.2.1`, password: `analog`).
...
* **Sample Rate:** `20 MSPS`
3. Load the `custom_ofdm_signal.bin` file (generated by the MATLAB helper script to ensure known seed synchronization).
4. Run the flowgraph.
